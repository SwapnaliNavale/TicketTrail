...
import com.tickettrail.security.JwtUtils;
...
public class UserController {
	...
	/*
	 * Desc - User signup URL - http://host:port/users/signup Method - POST payload
	 * - user reg dto Successful Resp - SC 201, user details - all (dto) Error resp
...
//	}
	...
	@PostMapping("/signin")
	public ResponseEntity<ApiResponse<AuthResponse>> userSignIn(@RequestBody @Valid AuthRequest dto) {
		System.out.println("in sign in " + dto);
		...
		// 4. Wrap response in ApiResponse
		ApiResponse<AuthResponse> response = new ApiResponse<>("success", new AuthResponse( token),
	...
	/*
	 * Desc - User signout URL - http://host:port/users/signout Method - POST
	 * Payload - JWT token (optional) Successful Resp - SC 200, message -
	 * "User signed out successfully" Error resp - SC 400, error message wrapped in
	 * DTO(ApiResponse)
	 */
	@PostMapping("/signout")
	public ResponseEntity<?> signOutUser(@RequestHeader("Authorization") String token) {
		...
		try {
			...
			return ResponseEntity.ok(new ApiResponse("success", "User signed out successfully"));
		} catch (RuntimeException e) {
	...
	/*
	 * Desc - Get User details URL - http://host:port/users/{userId} Method - GET
	 * 
	 * Successful Resp - SC 200, user details - all (dto) Error resp - SC 400 ,
	 * error mesg -wrapped in DTO(ApiResponse)
	 * 
	 */
	@GetMapping("/{userId}")
	...
	/*
	 * Desc - Get User details along with the blogs URL -
	 * http://host:port/users/{userId}/blogs Method - GET
	 * 
	 * Successful Resp - SC 200, user details + List <BlogDTO> Error resp - SC 400 ,
	 * error mesg -wrapped in DTO(ApiResponse)
	 * 
	 */
